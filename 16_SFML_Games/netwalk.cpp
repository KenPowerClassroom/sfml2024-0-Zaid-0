...
pipe grid[GRID_WIDTH][GRID_WIDTH];
pipe& cell(Vector2i v) {return grid[v.x][v.y];}
bool isOut(Vector2i v) {return !IntRect(0,0,GRID_WIDTH,GRID_WIDTH).contains(v);}


void generatePuzzle()
{
  std::vector<Vector2i> nodes;
  nodes.push_back(Vector2i(rand()%GRID_WIDTH,rand()%GRID_WIDTH));
...
int netwalk()
{
    ...
    Vector2i servPos;
    while(cell(servPos).dirs.size()==1) {servPos = Vector2i(rand()%GRID_WIDTH, rand()%GRID_WIDTH);}
